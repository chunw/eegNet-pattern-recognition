...
CLASS = cell(1,3); cindex = 1; %CLASS stores FINAL CLASSIFICATION RESULTS  
dataset_id = 0;
chan_num = 0;

for iter = 41:60 %'iter' is the row number of data to use
     if X(iter,4) ~= dataset_id || X(iter,5) ~= chan_num
        ...
        sample_data; %first-time window sliding and extracting features from each window
        class = KNN_classifier(W,W2,sample); %perform KNN classification
        
        sample_data2; %second-time window sliding and extracting features from each window
        class2 = KNN_classifier(W,W2,sample2); %perform KNN classification
        
       
        merge_class; 
        CLASS{cindex,3} = class; %merge results from 'class' and 'class2'
        cindex = cindex+1;
        count_detections;
