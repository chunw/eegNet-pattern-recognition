...
%LR(Linear Regression) training, learning weights associated with features
...
for iter = 41:60
     if X(iter,4) ~= dataset_id || X(iter,5) ~= chan_num
        ...
        class = LR(sample); %LINEAR REGRESSION METHOD
        
        
        sample_data2; %second time window sliding
        class2 = LR(sample2); %LINEAR REGRESSION METHOD
